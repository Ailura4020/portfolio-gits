...
const CustomCursor: React.FC = () => {
  ...
  const eyeGroupRef = useRef<SVGGElement>(null); // Référence pour l'ensemble du SVG
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (dotRef.current && outlineRef.current) {
        ...
        // 3. Calcul de l'angle entre l'oeil et le point
        const dx = mouseX.current - outlineX.current;
        const dy = mouseY.current - outlineY.current;
        const angle = Math.atan2(dy, dx);
        const angleDeg = angle * (180 / Math.PI);
        
        // 4. ORIENTATION GLOBALE DU SVG
        if (eyeGroupRef.current) {
          // On fait pivoter TOUT le groupe vers le curseur
  ...
  return (
    <>
      ...
      {/* CYBER-EYE (Structure complète pivotante) */}
      <div 
        ref={outlineRef}
        style={{
          position: 'fixed', top: 0, left: 0,
      ...
      >
        ...
        >
          {/* Groupe pivotant contenant TOUS les chemins du SVG */}
