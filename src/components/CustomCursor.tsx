...
const CustomCursor: React.FC = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const introElement = document.querySelector('.cyber-screen');
      const introActive = !!introElement;
      setIsInIntro(introActive);

      if (introActive) {
        if (isVisible) setIsVisible(false);
        return; // On arrête là si l'intro est présente
      }
    ...
    const animate = () => {
      // On n'anime que si on n'est pas dans l'intro
      if (dotRef.current && outlineRef.current && !document.querySelector('.cyber-screen')) {
        dotRef.current.style.transform = `translate3d(${mouseX.current}px, ${mouseY.current}px, 0)`;
        
        outlineX.current += (mouseX.current - outlineX.current) * 0.12;
        outlineY.current += (mouseY.current - outlineY.current) * 0.12;
        ...
        if (eyeGroupRef.current) {
          eyeGroupRef.current.style.transform = `rotate(${angleDeg}deg)`;
        }
      }
